			...
			function postIt() {
				...
				if(ret[0] && !rp_ge[$t.p.id].processing) {
					...
					if(postdata[oper] == "add"){
						if($($t).jqGrid('getGridParam','caption') == "用户表"){
							url = "userController/signUp.action";
						}
						else if($($t).jqGrid('getGridParam','caption') == "美发平台表"){
						...
						if($($t).jqGrid('getGridParam','caption') == "用户表"){
							url = "userController/updateUser.action";
						}
						else if($($t).jqGrid('getGridParam','caption') == "美发平台表"){
						...
						success:function(res){
							...
							if(res.msg == "ok"){
								if(postdata[oper] == "add"){
								 ...
								 data={readyState: 4, responseText: "", status: 200, statusText: "OK"};
		                         status="success";
							}
							else{
								data={readyState: 4, responseText: "", status: 500, statusText: "Error"};
		                        status="error";
							}
							var key;
			...
			if ( $("#"+$.jgrid.jqID(IDs.themodal))[0] !== undefined ) {
				...
				$("#dData","#"+dtbl+"_2").click(function(){
					...
					if(ret[0] && !rp_ge[$t.p.id].processing) {
						...
						//改动				
						if($($t).jqGrid('getGridParam','caption') == "用户表"){
								var url = "userController/deleteUser.action";
						}
						else if($($t).jqGrid('getGridParam','caption') == "美发平台表"){
							var url = "shopController/deleteShop.action";
						}
						else if($($t).jqGrid('getGridParam','caption') == "订单表"){
							var url = "ordersController/deleteOrders.action";
						}
	...
	saveRow : function(rowid, successfunc, url, extraparam, aftersavefunc,errorfunc, afterrestorefunc) {
		...
		if (editable==="1") {
			...
			if (o.url === 'clientArray') {
				...
				//改动
				if($($t).jqGrid('getGridParam','caption') == "用户表"){
					o.url = "userController/updateUser.action";
				}
				else if($($t).jqGrid('getGridParam','caption') == "美发平台表"){
					o.url = "shopController/updateShop.action";
				}
				$.ajax($.extend({
					url:o.url,
					...
					success:function(resp){
						var res,stat;
						if(resp.msg == "ok"){
							res={readyState: 4, responseText: "", status: 200, statusText: "OK"};
	                        stat="success";
						}
						else{
							res={readyState: 4, responseText: "", status: 500, statusText: "Error"};
							stat="error";
						}
